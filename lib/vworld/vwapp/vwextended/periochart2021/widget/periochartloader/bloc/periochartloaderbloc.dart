...
class PeriochartLoaderBloc
  ...
  Stream<PeriochartLoaderState> mapEventToState(
    ...
    if (event is LoadFromLocalHiveEvent) {
      yield PeriochartLoaderStateProcessing(
          title: 'Loading',
          description: 'Loading Records... ${event.loaderPcPropertiesId}');
      ...
      if (loadedPcProperties == null) {
        loadedPcProperties = Periochart.getBlankPcPropertiesId();
      }
      ...
      yield PeriochartLoaderStateShowChart(
          loaderPcPropertiesId: event.loaderPcPropertiesId,
          pcProperties: loadedPcProperties);
      return;
    } else if (event is SaveToLocalHiveEvent) {
      yield PeriochartLoaderStateProcessing(
          title: 'Loading',
          description: 'Saving Records... ${event.pcProperties.id}');
      ...
      yield PeriochartLoaderStateShowChart(
          loaderPcPropertiesId: event.pcProperties.id,
          pcProperties: event.pcProperties);
      return;
    }
  }
}
